<?php 
	...
	if(isset($_POST["add_to_cart"])) //when u click on button excute
	{
		 if(isset($_SESSION["s_cart"]))  // the session will store all the item data,used to check got dat or not in the session, if got data will execute 
      {  
           $product_array_id = array_column($_SESSION["s_cart"], "item_id");  //get the data of item it in array that in the session, get the array from the column data and coulmn name
           if(!in_array($_GET["id"], $product_array_id))  //used if function to to combine 2 varbile which is get the id from the URL and search from second parameter (product array_id)..//in array mean will judge the the item hv store in the session or not..// used to make the item wont add in 2 time
           {  //no allow the user to add the product 2 time in 1 purchase

                $count = count($_SESSION["s_cart"]);  //count the item 
                $product_array = array(  
                     'item_id'             =>     $_GET["id"], 
                ...
                $_SESSION["s_cart"][$count] = $product_array;  //this one is count
           }  
           else  
           {  
      ...
      {  
           $product_array = array(  //key word 'item_id' is get from the URL of html
                'item_id'               =>     $_GET["id"],  
                'item_name'               =>     $_POST["hidden_name"],  //item name get from hidden name and price also
                'item_price'          =>     $_POST["hidden_price"],  
                'item_quantity'          =>     $_POST["quantity"]  
           );  // can store all details of the item at this stage
           $_SESSION["s_cart"][0] = $product_array;  // store all item details to the session variable using array function , this one start from 0
      }  
 }  
	 ...
	 <div class="header-bottom">			
			...
			?>			
			 <div class="cart box_1">		
				...
				<p><a href="javascript:;" class="simpleCart_empty">View Cart</a></p>
...
<?php
	$sql ="SELECT * FROM tshirt ORDER BY tshirt_id ASC";
...
?>			
				<div class="col-md-3 feature-grid">
					...
					<center>	
                     ...
                     <input type="hidden" name="hidden_name" value="<?php echo $row["tshirt_name"]; ?>" />  <!--to store the price and name to add to cart-->
