...
<div class="product-model">	 
	...
	<table border="1" style="text-align:center" class="table table-striped"><!--table structure-->
		<tr.<!--for table header-->
			...
			<th style="width: 168px">Subject</th>
...
<?php
	...
	while($rows = mysqli_fetch_array($result))
	{
		...
		//to build a edit and dele te button
		echo "<td><a href='view.php?aid=".$rows['subject_id']."'>";
...
?>
   ...
   if(isset($_GET['aid'])){
   	$id =  $_GET['aid'];
   	$sql = ("DELETE FROM contact WHERE subject_id = $id");
	...
	//f.correct will come out success message
		...
		echo "<script>alert('The Feedback sucessfully deleted from the database!');</script>";
